...
<head>
  ...
  <script>
    ...
    function resetCardKey() {
      if (!confirm("This will restore the RFID card key to the factory default.\nPresent the card after clicking OK.")) return;
      document.getElementById("message").innerHTML = "<label class=\"msg\">Waiting for card — hold it to the reader...</label>";
      var xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function () {
        if (this.readyState == 4) {
          if (this.status == 200) {
            document.getElementById("message").innerHTML = "<label class=\"msg\">" + this.responseText + "</label>";
          } else {
      ...
      xhr.open("POST", "/resetcard", true);
      xhr.send();
    }
    function readCardData() {
      document.getElementById("message").innerHTML = "<label class=\"msg\">Reading card... hold card to reader</label>";
      document.getElementById("cardDump").textContent = "";
      var xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function () {
        if (this.readyState == 4) {
          if (this.status == 200) {
            document.getElementById("message").innerHTML = "<label class=\"msg\">Card read complete</label>";
            var matchLine = "MatchesCurrentSpoolData=UNKNOWN";
            var lines = this.responseText.split("\n");
            for (var i = 0; i < lines.length; i++) {
              if (lines[i].indexOf("MatchesCurrentSpoolData=") === 0) {
                matchLine = lines[i];
                break;
              }
            }
            document.getElementById("cardDump").textContent = matchLine;
          } else {
            document.getElementById("message").innerHTML = "<label class=\"errormsg\">Read error: " + this.responseText + "</label>";
            document.getElementById("cardDump").textContent = "MatchesCurrentSpoolData=ERROR";
          }
        }
      };
      xhr.open("POST", "/readcard", true);
      xhr.send();
    }
    function sendData() {
...
<body>
  <center>
    <div class="main">
      ...
      <pre id="cardDump" class="statusbox">MatchesCurrentSpoolData=UNKNOWN</pre>
      <br>
      <div class="button-row">
        <input type="submit" class="btn" value="Reset" onClick="clearCache()">
        <input type="submit" class="btn" value="Apply" onClick="sendData()">
      </div>
      <div class="button-row">
        <input type="button" class="btn" value="Read Card" onClick="readCardData()">
        <input type="button" class="btn" style="background-color:#c0392b" value="Reset Card Key" onClick="resetCardKey()">
      </div>
    </div>
  </center>
  <dialog id="cDialog">
